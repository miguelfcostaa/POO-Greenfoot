...
public class Floresta extends Actor
{
     ...
     */
    private Text ind=new Text();            //cria uma referencia para o Text
    public void act(){
        ...
        if (isTouching(Boy.class) || isTouching(Girl.class)){
            ...
            if (Greenfoot.isKeyDown("E") && Greenfoot.isKeyDown("I")){
                ...
                Calhau c = new Calhau();
                Score scr = new Score();
                Greenfoot.setWorld(new CortaArvore(hb.getHealthB(),hg.getHealthG(),fb.getFoodB(),fg.getFoodG(),wb.getWaterB(),wg.getWaterG(),m.getMadeira(),c.getPedra(),scr.getScore()));
            }
        }
        if(!isTouching(Boy.class) || isTouching(Girl.class)){
            ind.setText("");
